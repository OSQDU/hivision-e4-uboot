	...
	while (totlen) {
		...
		if (res < 0) {
			if (mtd_is_eccerr(res)) {
				printf("spi_nand_bbt: ECC error in BBT at 0x%012llx\n",
					from & ~mtd->writesize);
				return res;
			} else if (mtd_is_bitflip(res)) {
				printf("spi_nand_bbt: corrected error in BBT at 0x%012llx\n",
					from & ~mtd->writesize);
				ret = res;
			} else {
				printf("spi_nand_bbt: error reading BBT\n");
				return res;
			}
		}
		...
		for (i = 0; i < len; i++) {
			...
			for (j = 0; j < 8; j += bits, act++) {
				...
				if (reserved_block_code &&
					...
					fh_dev_debug("spi_nand_read_bbt: reserved block at 0x%012llx\n",
						(loff_t)(offs + act) <<
						this->block_shift);
				 ...
				 * move this message to printf.
				 */
				fh_dev_debug("spi_nand_read_bbt: bad block at 0x%012llx\n",
					(loff_t)(offs + act) <<
					this->block_shift);
			  ...
			  struct nand_bbt_descr *td, struct nand_bbt_descr *md)
{
	...
	if (td->options & NAND_BBT_VERSION) {
		...
		fh_dev_debug("Bad block table at page %d, version 0x%02X\n",
		      ...
		      struct nand_bbt_descr *bd, int chip)
{
	...
	for (i = startblock; i < numblocks; i++) {
		...
		if (ret) {
			...
			fh_dev_debug("Bad eraseblock %d at 0x%012llx\n",
		      ...
		      struct nand_bbt_descr *td)
{
	...
	for (i = 0; i < chips; i++) {
		if (td->pages[i] == -1)
			fh_dev_debug("Bad block table not found for chip %d\n", i);
		else
			fh_dev_debug("Bad block table found at page %d, version 0x%02X\n",
...
write:
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			if (res < 0) {
				if (retlen != len) {
					printf("spi_nand_bbt: error reading block ");
					printf("for writing the bad block table\n");
					return res;
				}
				printf("spi_nand_bbt: ECC error while reading ");
				printf("block for writing bad block table\n");
			}
			/* Read oob data */
		...
		printf("Bad block table written to 0x%012llx, version 0x%02X\n",
...
outerr:
	printf("spi_nand_bbt: error while writing bad block table %d\n", res);
	return res;
}
...
int spi_nand_scan_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	...
	if (!td) {
		...
		if (res) {
			printf("spi_nand_bbt: can't scan flash and build the RAM-based BBT\n");
			kfree(this->bbt);
			this->bbt = NULL;
		}
		return res;
...
static int spi_nand_create_badblock_pattern(struct spi_nand_chip *this)
{
	...
	if (this->badblock_pattern) {
		printf("Bad block pattern already allocated; not replacing\n");
		return -EINVAL;
	}
...
int spi_nand_isbad_bbt(struct mtd_info *mtd, loff_t offs, int allowbbt)
{
	...
	fh_dev_debug("%s: bbt info for offs 0x%08x: (block %d) 0x%02x\n",
