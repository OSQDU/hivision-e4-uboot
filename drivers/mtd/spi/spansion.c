...
#define CMD_S25FLXX_RDCR    0x35    /* Read Status Register */
...
#define SPANSION_SR_WEL     (1 << 1)
#define SPANSION_SR_BP0     (1 << 2)
#define SPANSION_SR_BP1     (1 << 3)
#define SPANSION_SR_BP2     (1 << 4)
#define SPANSION_SR_EERR    (1 << 5)
#define SPANSION_SR_PERR    (1 << 6)
#define SPANSION_SR_SRWD    (1 << 7)

#define SPANSION_CR_FREEZE  (1 << 0)    /* Write-in-Progress */
#define SPANSION_CR_QUAD    (1 << 1)
#define SPANSION_CR_TBPARM  (1 << 2)
#define SPANSION_CR_BPNV    (1 << 3)
#define SPANSION_CR_RFU     (1 << 4)
#define SPANSION_CR_TBPROT  (1 << 5)
#define SPANSION_CR_LC0     (1 << 6)
#define SPANSION_CR_LC1     (1 << 7)
...
int spansion_protect(struct spi_flash *flash, int on)
{
    struct spi_slave *spi = flash->spi;
    int ret;
    u8 status1, status2;
    ...
    /* read status 1 */
    ret = spi_flash_cmd(flash->spi, CMD_S25FLXX_RDSR, &status1, 1);
    if (ret < 0) {
        debug("SF: Enabling Write failed\n");
        goto out;
    }

    if(status1 & SPANSION_SR_SRWD)
    {
    ...
    /* read status 2 */
    ret = spi_flash_cmd(flash->spi, CMD_S25FLXX_RDCR, &status2, 1);
    if (ret < 0) {
        debug("SF: Enabling Write failed\n");
        goto out;
    }

    if(status2 & SPANSION_CR_FREEZE)
    {
        printf("flash status register is protected, please set FREEZE to low in order to unprotected it.\n");
        goto out;
    }
    ...
    if(on)
    {
        status1 |= (SPANSION_SR_BP0);
        status1 &= (~(SPANSION_SR_BP1 | SPANSION_SR_BP2));
        status2 |= SPANSION_CR_TBPROT;
    }
    else
    {
        status1 = 0;
...
struct spi_flash *spi_flash_probe_spansion(struct spi_slave *spi, u8 *idcode)
{
	...
	spsn->flash.protect = spansion_protect;
